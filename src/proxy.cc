...
static const wchar_t* szDllName = L"version";
...
load_actual_library()
{
	...
	{
		GetSystemWow64DirectoryW(szPath, MAX_PATH);
	}

	unload_actual_library();

	wchar_t szDllPath[MAX_PATH];
	swprintf_s(szDllPath, MAX_PATH, L"%s\\%s.dll", szPath, szDllName);
...
template<typename T>
void
_setup_hook(T*& func, const char* name)
{
	if (func != nullptr)
		return;
...
// Ordinal           : 2 (0x2)
extern "C" int WINAPI
GetFileVersionInfoByHandle(int hMem, LPCWSTR lpFileName, int v2, int v3)
{
...
// Ordinal           : 3 (0x3)
extern "C" BOOL WINAPI
GetFileVersionInfoExA(DWORD dwFlags, LPCSTR lpwstrFilename, DWORD dwHandle, DWORD dwLen, LPVOID lpData)
{
...
// Ordinal           : 4 (0x4)
extern "C" BOOL WINAPI
GetFileVersionInfoExW(DWORD dwFlags, LPCWSTR lpwstrFilename, DWORD dwHandle, DWORD dwLen, LPVOID lpData)
{
...
// Ordinal           : 6 (0x6)
extern "C" DWORD WINAPI
GetFileVersionInfoSizeExA(DWORD dwFlags, LPCSTR lpwstrFilename, LPDWORD lpdwHandle)
{
...
// Ordinal           : 7 (0x7)
extern "C" DWORD WINAPI
GetFileVersionInfoSizeExW(DWORD dwFlags, LPCWSTR lpwstrFilename, LPDWORD lpdwHandle)
{
...
// Ordinal           : 14 (0xe)
extern "C" DWORD WINAPI
VerLanguageNameA(DWORD wLang, LPSTR szLang, DWORD cchLang)
{
...
// Ordinal           : 15 (0xf)
extern "C" DWORD WINAPI
VerLanguageNameW(DWORD wLang, LPWSTR szLang, DWORD cchLang)
{
